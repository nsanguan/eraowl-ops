...
async def get_user(user_id: uuid.UUID, svc: AdminService = Depends(get_service)):
    return await svc.get_user(user_id)


@router.post("/users", response_model=schemas.UserOut, status_code=status.HTTP_201_CREATED)
async def create_user(data: schemas.UserCreate, svc: AdminService = Depends(get_service)):
...
async def delete_user(user_id: uuid.UUID, svc: AdminService = Depends(get_service)):
    await svc.delete_user(user_id)


@router.put("/users/{user_id}/deactivate", response_model=schemas.UserOut)
async def deactivate_user(user_id: uuid.UUID, svc: AdminService = Depends(get_service)):
...
async def get_user_roles(user_id: uuid.UUID, svc: AdminService = Depends(get_service)):
...
async def get_user_business_units(
    user_id: uuid.UUID, svc: AdminService = Depends(get_service)
):
...
async def get_role(role_id: uuid.UUID, svc: AdminService = Depends(get_service)):
    return await svc.get_role(role_id)


@router.post("/roles", response_model=schemas.RoleOut, status_code=status.HTTP_201_CREATED)
async def create_role(data: schemas.RoleCreate, svc: AdminService = Depends(get_service)):
...
async def delete_role(role_id: uuid.UUID, svc: AdminService = Depends(get_service)):
...
async def get_role_permissions(
    role_id: uuid.UUID, svc: AdminService = Depends(get_service)
):
...
async def list_privileges(svc: AdminService = Depends(get_service)):
    return await svc.list_privileges()


@router.post("/privileges/seed")
async def seed_privileges(svc: AdminService = Depends(get_service)):
