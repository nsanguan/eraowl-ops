...
class AdminService:
    ...
    async def list_users(
        ...
        result = await self.db.execute(
            select(User).where(where).order_by(User.created_at.desc()).offset(offset).limit(page_size)
        )
        users = list(result.scalars().all())

        items = []
        for u in users:
            roles = await self.get_user_roles(u.user_id)
            items.append({
    ...
    async def delete_user(self, user_id: uuid.UUID) -> None:
        ...
        # Write audit log BEFORE hard delete
        await self._write_audit_log(
            user_id=user_id,
            action="user_deleted",
            target_entity="User",
