...
from app.modules.po.models import PoHeader
...
async def get_po_header(po_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = GenericPoService(db)
...
async def create_po_header(data: CreatePoHeaderRequest, db: AsyncSession = Depends(get_db)):
    svc = GenericPoService(db)
    return await svc.create("po_headers", data.model_dump())


@router.put("/po_headers/{po_id}", response_model=PoHeaderResponse)
async def update_po_header(po_id: uuid.UUID, data: UpdatePoHeaderRequest, db: AsyncSession = Depends(get_db)):
    svc = GenericPoService(db)
...
async def delete_po_header(po_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = GenericPoService(db)
...
async def get_po_line(line_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoLineService(db)
    return await svc.get(line_id)


@router.post("/po_lines", response_model=PoLineResponse, status_code=status.HTTP_201_CREATED)
async def create_po_line(data: CreatePoLineRequest, db: AsyncSession = Depends(get_db)):
    svc = PoLineService(db)
    return await svc.create(data)


@router.put("/po_lines/{line_id}", response_model=PoLineResponse)
async def update_po_line(line_id: uuid.UUID, data: UpdatePoLineRequest, db: AsyncSession = Depends(get_db)):
    svc = PoLineService(db)
    return await svc.update(line_id, data)


@router.delete("/po_lines/{line_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_po_line(line_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoLineService(db)
...
async def get_po_shipment(shipment_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoShipmentService(db)
...
async def create_po_shipment(data: CreatePoShipmentRequest, db: AsyncSession = Depends(get_db)):
    svc = PoShipmentService(db)
    return await svc.create(data)


@router.put("/po_shipments/{shipment_id}", response_model=PoShipmentResponse)
async def update_po_shipment(shipment_id: uuid.UUID, data: UpdatePoShipmentRequest, db: AsyncSession = Depends(get_db)):
    svc = PoShipmentService(db)
    return await svc.update(shipment_id, data)


@router.delete("/po_shipments/{shipment_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_po_shipment(shipment_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoShipmentService(db)
...
async def get_po_distribution(dist_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoDistributionService(db)
...
async def create_po_distribution(data: CreatePoDistributionRequest, db: AsyncSession = Depends(get_db)):
    svc = PoDistributionService(db)
    return await svc.create(data)


@router.put("/po_distributions/{dist_id}", response_model=PoDistributionResponse)
async def update_po_distribution(dist_id: uuid.UUID, data: UpdatePoDistributionRequest, db: AsyncSession = Depends(get_db)):
    svc = PoDistributionService(db)
    return await svc.update(dist_id, data)


@router.delete("/po_distributions/{dist_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_po_distribution(dist_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoDistributionService(db)
...
async def get_po_release(release_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoReleaseService(db)
...
async def create_po_release(data: CreatePoReleaseRequest, db: AsyncSession = Depends(get_db)):
    svc = PoReleaseService(db)
    return await svc.create(data)


@router.put("/po_releases/{release_id}", response_model=PoReleaseResponse)
async def update_po_release(release_id: uuid.UUID, data: UpdatePoReleaseRequest, db: AsyncSession = Depends(get_db)):
    svc = PoReleaseService(db)
    return await svc.update(release_id, data)


@router.delete("/po_releases/{release_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_po_release(release_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoReleaseService(db)
...
async def get_po_amendment(amendment_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoAmendmentService(db)
...
async def create_po_amendment(data: CreatePoAmendmentRequest, db: AsyncSession = Depends(get_db)):
    svc = PoAmendmentService(db)
    return await svc.create(data)


@router.put("/po_amendments/{amendment_id}", response_model=PoAmendmentResponse)
async def update_po_amendment(amendment_id: uuid.UUID, data: UpdatePoAmendmentRequest, db: AsyncSession = Depends(get_db)):
    svc = PoAmendmentService(db)
    return await svc.update(amendment_id, data)


@router.delete("/po_amendments/{amendment_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_po_amendment(amendment_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoAmendmentService(db)
...
async def get_po_approval(approval_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoApprovalService(db)
...
async def create_po_approval(data: CreatePoApprovalRequest, db: AsyncSession = Depends(get_db)):
    svc = PoApprovalService(db)
    return await svc.create(data)


@router.put("/po_approvals/{approval_id}", response_model=PoApprovalResponse)
async def update_po_approval(approval_id: uuid.UUID, data: UpdatePoApprovalRequest, db: AsyncSession = Depends(get_db)):
    svc = PoApprovalService(db)
    return await svc.update(approval_id, data)


@router.delete("/po_approvals/{approval_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_po_approval(approval_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    svc = PoApprovalService(db)
...
async def get_po_composite(po_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def create_po_composite(data: CreatePoCompositeRequest, db: AsyncSession = Depends(get_db)):
    svc = PoCompositeService(db)
    return await svc.create_composite(data)


@router.put("/po_headers/{po_id}/composite")
async def update_po_composite(po_id: uuid.UUID, data: UpdatePoCompositeRequest, db: AsyncSession = Depends(get_db)):
    svc = PoCompositeService(db)
