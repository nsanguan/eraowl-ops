...
resolve_function_name() {
  if [[ -n "$FUNCTION_NAME" ]]; then
    case "$FUNCTION_NAME_SOURCE" in
      ...
      "")
        if ! is_interactive; then
          emit_kv function_name_state missing
...
resolve_function_name
emit_kv function_name_state "$FUNCTION_NAME_STATE"
emit_kv function_name "$FUNCTION_NAME"
...
if [[ "$SKIP_PREFLIGHT" != "true" ]]; then
  ...
  emit_kv deploy_preflight_state passed
else
  skip_stdout=""
  skip_stderr=""
  skip_status=""
  ...
  emit_kv deploy_preflight_state skipped
fi
