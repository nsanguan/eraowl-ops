...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  emit_kv app_error_reason runtime_or_cli
...
all_apps_out=""
all_apps_err=""
if ! run_oci_display_name_list all_apps_out all_apps_err fn application list --compartment-id "$COMPARTMENT_ID" --all; then
  emit_kv apps_state error
  emit_kv apps_error_reason "$(classify_oci_error_reason "$all_apps_err")"
  emit_kv app_state error
  emit_kv app_error_reason "$(classify_oci_error_reason "$all_apps_err")"
...
if [[ ${#existing_apps[@]} -gt 0 ]]; then
  ...
  emit_kv apps_count "${#existing_apps[@]}"
  idx=0
  for item in "${existing_apps[@]}"; do
    idx=$((idx + 1))
    emit_kv "app_item_${idx}" "$item"
  done
else
...
app_lookup_out=""
app_lookup_err=""
if run_oci_display_name_lookup app_lookup_out app_lookup_err "$APP_NAME" fn application list --compartment-id "$COMPARTMENT_ID" --all; then
  app_id="$app_lookup_out"
else
  emit_kv app_state error
  emit_kv app_error_reason "$(classify_oci_error_reason "$app_lookup_err")"
...
if [[ -n "$app_id" && "$app_id" != "null" && "$app_id" != "None" ]]; then
  ...
  if [[ "$APP_CHOICE" != "reuse" ]]; then
    ...
    emit_kv app_name "$APP_NAME"
    emit_kv app_id "$app_id"
    emit_kv app_action reuse
    exit 0
  fi
fi
