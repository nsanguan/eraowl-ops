...
if [[ -z "$OS_NAME" ]]; then
  case "$(uname -s)" in
    ...
    *)
      emit_kv install_state error
      emit_kv install_error_reason unsupported_os
...
case "$OS_NAME:$TOOL" in
  ...
  linux:fn) install_method="verified_release" ;;
  linux:oci)
    if [[ -n "$PKG_MGR" ]]; then
...
if [[ "$OS_NAME" == "darwin" && "$install_method" != "app_start" && "$has_brew" != "yes" ]]; then
  emit_kv install_state error
  emit_kv install_error_reason missing_package_manager
...
if [[ -z "$install_method" ]]; then
  emit_kv install_state error
  emit_kv install_error_reason unsupported_package_manager
...
case "$OS_NAME:$TOOL:$install_method" in
  ...
  darwin:docker-daemon:app_start)
    ...
    ;;
  linux:fn:verified_release)
    fn_release_version=""
    fn_release_url=""
    fn_release_digest=""
    if ! resolve_latest_fn_linux_release fn_release_version fn_release_url fn_release_digest; then
      emit_kv install_state error
      emit_kv install_error_reason release_metadata_validation_failed
      exit 4
    fi
    display_command="scripts/install_fn_linux.sh --version '$fn_release_version' --asset-url '$fn_release_url' --sha256 '$fn_release_digest'"
    run_confirmed \
      "Install/fix '$TOOL' on '$OS_NAME' using verified Fn CLI release '$fn_release_version' with digest '$fn_release_digest'" \
      "$display_command" \
      "$SCRIPT_DIR/install_fn_linux.sh" \
      --version "$fn_release_version" \
      --asset-url "$fn_release_url" \
      --sha256 "$fn_release_digest"
    ;;
  ...
  *)
    emit_kv install_state error
    emit_kv install_error_reason unsupported_install_method
...
if [[ "$TOOL" == "docker-daemon" ]]; then
  if docker info >/dev/null 2>&1; then
    ...
    emit_kv install_state success
    emit_kv install_method "$install_method"
  else
    emit_kv install_state error
    emit_kv install_error_reason verification_failed
