...
resolve_id_with_retry() {
  ...
  for ((i=1; i<=retries; i++)); do
    if run_oci_display_name_lookup out err "$display_name" "$@"; then
      value="$out"
    else
      value=""
    fi
  ...
  if [[ -n "$err" ]]; then
    log_warn "Resource lookup retry ended with: $err"
  fi
  printf -v "$__out_var" "%s" "$value"
}
...
if ! command -v oci >/dev/null 2>&1; then
  emit_kv network_state error
  emit_kv network_error_reason runtime_or_cli
...
if [[ "$ALLOW_NETWORK_CREATE" != "yes" ]]; then
  emit_kv network_state error
  emit_kv network_error_reason missing_opt_in
...
if ! is_interactive; then
  for required_flag in TOPOLOGY VCN_NAME VCN_CIDR SUBNET_NAME SUBNET_CIDR; do
    if [[ -z "${!required_flag:-}" ]]; then
      emit_kv network_state error
      emit_kv network_error_reason missing_required_input
...
all_vcns_out=""
vcn_err=""
if run_oci_display_name_list all_vcns_out vcn_err network vcn list --compartment-id "$COMPARTMENT_ID" --all; then
  if [[ -n "$(sed '/^$/d' <<<"$all_vcns_out")" ]]; then
    emit_kv vcns_state found
  else
    emit_kv vcns_state empty
  fi
else
  emit_kv vcns_state error
  emit_kv network_state error
  emit_kv network_error_reason "$(classify_oci_error_reason "$vcn_err")"
...
vcn_id=""
vcn_lookup=""
if run_oci_display_name_lookup vcn_lookup vcn_err "$VCN_NAME" network vcn list --compartment-id "$COMPARTMENT_ID" --all; then
  vcn_id="$vcn_lookup"
else
  emit_kv network_state error
  emit_kv network_error_reason "$(classify_oci_error_reason "$vcn_err")"
...
PY
  ...
  resolve_id_with_retry vcn_id 6 2 "$VCN_NAME" network vcn list --compartment-id "$COMPARTMENT_ID" --all
fi
...
if [[ "$TOPOLOGY" == "public" || "$TOPOLOGY" == "mixed" ]]; then
  ...
  igw_err=""
  if ! run_oci_display_name_lookup igw_id igw_err "$igw_name" network internet-gateway list --compartment-id "$COMPARTMENT_ID" --all; then
    echo "[ERROR] Unable to inspect internet gateways: $igw_err" >&2
    exit 4
  fi
  if [[ -z "$igw_id" || "$igw_id" == "null" || "$igw_id" == "None" ]]; then
    ...
    if ! run_oci_display_name_lookup igw_id igw_err "$igw_name" network internet-gateway list --compartment-id "$COMPARTMENT_ID" --all; then
      echo "[ERROR] Unable to resolve the created internet gateway." >&2
      exit 4
    fi
  fi
fi
...
if [[ "$TOPOLOGY" == "private" || "$TOPOLOGY" == "mixed" ]]; then
  ...
  sgw_err=""
  if ! run_oci_display_name_lookup sgw_id sgw_err "$sgw_name" network service-gateway list --compartment-id "$COMPARTMENT_ID" --all; then
    echo "[ERROR] Unable to inspect service gateways: $sgw_err" >&2
    exit 4
  fi
...
PY
  ...
  if [[ -z "$sgw_id" || "$sgw_id" == "null" || "$sgw_id" == "None" ]]; then
    ...
    if ! run_oci_display_name_lookup sgw_id sgw_err "$sgw_name" network service-gateway list --compartment-id "$COMPARTMENT_ID" --all; then
      echo "[ERROR] Unable to resolve the created service gateway." >&2
      exit 4
    fi
  fi
fi
...
route_table_err=""
if ! run_oci_display_name_lookup route_table_id route_table_err "$rt_name" network route-table list --compartment-id "$COMPARTMENT_ID" --vcn-id "$vcn_id" --all; then
  echo "[ERROR] Unable to inspect route tables: $route_table_err" >&2
  exit 4
fi
...
if [[ -z "$route_table_id" || "$route_table_id" == "null" || "$route_table_id" == "None" ]]; then
  if ! run_oci_display_name_lookup route_table_id route_table_err "$rt_name" network route-table list --compartment-id "$COMPARTMENT_ID" --vcn-id "$vcn_id" --all; then
    echo "[ERROR] Unable to inspect route tables." >&2
    exit 4
  fi
  if [[ -z "$route_table_id" || "$route_table_id" == "null" || "$route_table_id" == "None" ]]; then
    ...
    resolve_id_with_retry route_table_id 6 2 "$rt_name" network route-table list --compartment-id "$COMPARTMENT_ID" --vcn-id "$vcn_id" --all
  else
...
subnet_err=""
if ! run_oci_display_name_lookup subnet_id subnet_err "$SUBNET_NAME" network subnet list --compartment-id "$COMPARTMENT_ID" --vcn-id "$vcn_id" --all; then
  echo "[ERROR] Unable to inspect subnets: $subnet_err" >&2
  exit 4
fi
if [[ -z "$subnet_id" || "$subnet_id" == "null" || "$subnet_id" == "None" ]]; then
  ...
  resolve_id_with_retry subnet_id 6 2 "$SUBNET_NAME" network subnet list --compartment-id "$COMPARTMENT_ID" --vcn-id "$vcn_id" --all
else
...
emit_kv network_state found
emit_kv vcn_id "$vcn_id"
emit_kv subnet_id "$subnet_id"
