...
check_cmd() {
  ...
  if command -v "$name" >/dev/null 2>&1; then
    ...
    emit_kv "dependency_${state_key}_state" found
  else
    log_missing "$name"
    emit_kv "dependency_${state_key}_state" missing
    missing+=("$name")
  fi
}
...
if command -v docker >/dev/null 2>&1; then
  if docker info >/dev/null 2>&1; then
    ...
    emit_kv docker_daemon_state running
  else
    log_warn "Docker installed but daemon is not running"
    emit_kv docker_daemon_state stopped
    warns+=("docker-daemon")
  fi
else
  emit_kv docker_daemon_state missing
fi
...
if [[ -n "$PROFILE_FALLBACK" ]]; then
  ...
  emit_kv oci_profile_source explicit_flag
elif [[ -n "$FN_CONTEXT_PROFILE" ]]; then
  resolved_profile="$FN_CONTEXT_PROFILE"
  emit_kv oci_profile_source fn_context
else
  emit_kv oci_profile_source missing
fi
emit_kv oci_profile "${resolved_profile:-}"
...
if command -v oci >/dev/null 2>&1; then
  ...
  if [[ -n "$resolved_profile" ]]; then
    ...
    if run_oci_capture _oci_out oci_err iam region list --profile "$resolved_profile" --all; then
      ...
      emit_kv oci_auth_state valid
      emit_kv oci_auth_mode default
    else
      retry_succeeded="no"
      if grep -Eiq 'security[_ -]?token|session token|token-based profile|must use.*security_token|use.*security_token|OCI_CLI_AUTH=security_token' <<<"$oci_err"; then
        emit_kv oci_auth_retry_state available
          ...
          case "$(normalize_reply "$retry_reply")" in
            y|yes)
              if OCI_CLI_AUTH=security_token run_oci_capture _retry_out _retry_err iam region list --profile "$resolved_profile" --all; then
                ...
                emit_kv oci_auth_state valid
                emit_kv oci_auth_mode security_token
                retry_succeeded="yes"
              else
                log_warn "OCI profile '$resolved_profile' still failed with security_token auth"
                emit_kv oci_auth_retry_state failed
                warns+=("oci-profile:$resolved_profile")
              fi
              ;;
            *)
              emit_kv oci_auth_retry_state declined
              warns+=("oci-profile:$resolved_profile")
              ;;
          esac
        else
          emit_kv oci_auth_retry_state approval_required
          warns+=("oci-profile:$resolved_profile")
        fi
      fi
      ...
      if [[ "$retry_succeeded" != "yes" ]]; then
        ...
        emit_kv oci_auth_state error
        emit_kv oci_auth_error_reason "$(classify_oci_error_reason "$oci_err")"
        emit_kv oci_auth_error "$(tr '\n' ' ' <<<"$oci_err" | sed 's/[[:space:]]\+/ /g')"
        warns+=("oci-profile:$resolved_profile")
      fi
    fi
  else
    log_warn "No OCI profile could be resolved from Fn context"
    emit_kv oci_auth_state missing
...
if [[ -n "$FN_CONTEXT_REGISTRY" ]]; then
  ...
  emit_kv docker_registry_host "$registry_host"
  ...
  case "$registry_auth_state" in
    valid)
      ...
      emit_kv docker_registry_auth_state valid
      emit_kv docker_registry_auth_probe_state valid
      ;;
    cached)
      log_warn "Docker has cached auth for '$registry_host', but live validation did not confirm it"
      emit_kv docker_registry_auth_state cached
      emit_kv docker_registry_auth_probe_state cached
      warns+=("docker-auth:$registry_host")
      ;;
    error)
      emit_kv docker_registry_auth_state error
      emit_kv docker_registry_auth_probe_state error
      emit_kv docker_registry_auth_error_reason runtime_or_cli
      warns+=("docker-auth:$registry_host")
      ;;
    *)
      log_warn "Docker is not logged in to '$registry_host'"
      emit_kv docker_registry_auth_state missing
      emit_kv docker_registry_auth_probe_state missing
      warns+=("docker-auth:$registry_host")
      ;;
  esac
else
