...
emit_kv fn_context_name "${CURRENT_CONTEXT_NAME:-$CONTEXT_NAME}"
emit_kv fn_context_profile "$PROFILE"
emit_kv fn_context_compartment_id "$COMPARTMENT_ID"
emit_kv fn_context_api_url "$API_URL"
emit_kv fn_context_registry "$REGISTRY"
emit_kv fn_context_image_compartment_id "${IMAGE_COMPARTMENT_ID:-}"

registry_host="${REGISTRY%%/*}"
emit_kv docker_registry_host "$registry_host"
registry_auth_state="$(docker_registry_auth_probe "$registry_host")"
emit_kv docker_registry_auth_probe_state "$registry_auth_state"
  ...
  case "$registry_auth_state" in
    ...
    cached)
      if [[ -z "$OCIR_USERNAME" ]]; then
        ...
        emit_kv ocir_auth_state cached
        exit 0
      fi
      ;;
    error)
...
if [[ -n "$OCIR_USERNAME" ]]; then
  ...
  emit_kv ocir_auth_state missing
fi
