...
const CUSTOMER_CLASSES = ['STANDARD', 'GOVERNMENT', 'EMPLOYEE', 'ONE_TIME', 'INTERNAL', 'FOREIGN']
...
export default function CustomerPage() {
  ...
  const isFormOpen = Boolean(customerId) || location.state?.creating === true
  ...
  const [form, setForm] = useState({
    customer_code: '', customer_class_code: 'STANDARD', credit_limit: '',
    payment_term_days: '', is_active: true,
  })
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('/party/customers', { params: { page: 1, page_size: 100 } })
      setCustomers(parseRows(data))
  ...
  const fetchExisting = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(`/party/customers/${customerId}`)
      setExistingCustomer(data)
      setForm({
        customer_code: data.customer_code || '',
        customer_class_code: data.customer_class_code || 'STANDARD',
  ...
  const fetchSites = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('/party/customer-sites', { params: { customer_id: customerId, page: 1, page_size: 100 } })
  ...
  useEffect(() => { if (!isFormOpen) fetchCustomers() }, [isFormOpen, fetchCustomers])
  ...
  const handleSave = useCallback(async () => {
    if (!form.customer_code.trim()) { alert('Customer code is required.'); return }
    setSaving(true)
    try {
      const payload = {}
      for (const k of Object.keys(form)) {
        if (k === 'credit_limit') payload[k] = form[k] ? parseFloat(form[k]) : undefined
        else if (k === 'payment_term_days') payload[k] = form[k] ? parseInt(form[k]) : undefined
        else if (k === 'is_active') payload[k] = form[k]
        else payload[k] = form[k] || undefined
      }
      if (isEdit && customerId) {
        await api.put(`/party/customers/${customerId}`, payload)
      } else {
        await api.post('/party/customers', payload)
      }
      navigate('/party/customers', { replace: true })
    } catch (err) {
      alert(err.response?.data?.detail?.message || 'Failed to save')
    } finally { setSaving(false) }
  }, [form, isEdit, customerId, navigate])

  const handleDelete = useCallback(async (id) => {
    if (!confirm('Delete this customer?')) return
    try {
      await api.delete(`/party/customers/${id}`)
      fetchCustomers()
    } catch (err) { alert(err.response?.data?.detail?.message || 'Failed to delete') }
  }, [fetchCustomers])

  const siteUpdate = useCallback(async (id, payload) => {
    try { await api.put(`/party/customer-sites/${id}`, payload); fetchSites() } catch (err) { alert(err.response?.data?.detail?.message || 'Failed') }
  }, [fetchSites])

  const siteCreate = useCallback(async (payload) => {
    try { await api.post('/party/customer-sites', { ...payload, customer_id: customerId }); setNewSite(null); fetchSites() } catch (err) { alert(err.response?.data?.detail?.message || 'Failed') }
  }, [customerId, fetchSites])

  const siteDelete = useCallback(async (id) => {
    try { await api.delete(`/party/customer-sites/${id}`); fetchSites() } catch (err) { alert(err.response?.data?.detail?.message || 'Failed') }
  }, [fetchSites])
  ...
  if (!isFormOpen) {
    const listColumns = [
      {
        key: 'customer_code', header: 'Customer Code', width: '160px',
        render: (row) => (
          <button onClick={() => navigate(`/party/customers/${row.customer_id}`)}
            className="text-sm font-semibold text-primary hover:underline text-left">{row.customer_code}</button>
        )
      },
      { key: 'customer_class_code', header: 'Class', width: '120px',
        render: (row) => <span className="text-[11px] font-semibold px-2 py-1 rounded-md bg-primary/10 text-primary">{(row.customer_class_code || 'STANDARD').replace(/_/g, ' ')}</span>
      },
      { key: 'credit_limit', header: 'Credit Limit', width: '120px',
        render: (row) => <span className="text-sm font-mono text-on-surface">{row.credit_limit != null ? Number(row.credit_limit).toLocaleString() : '\u2014'}</span>
      },
    ...
    return (
      <div className="h-[calc(100vh-6.5rem)] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-outline-variant">
          <div>
            <h1 className="text-[32px] font-semibold leading-10 tracking-tight text-slate-900! dark:text-white!">Customers</h1>
            <p className="text-sm text-slate-500! dark:text-slate-300! mt-1">{customersLoading ? 'Loading...' : `${customers.length} customer${customers.length !== 1 ? 's' : ''}`}</p>
          </div>
          <button onClick={() => navigate('/party/customers', { state: { creating: true } })}
            ...
            <span className="material-symbols-outlined !text-[16px]">add</span> Add New Customer</button>
        </div>
        <div className="flex-1 overflow-y-auto p-4">
          <InteractiveGrid
            ...
            idKey="customer_id"
            searchable
            onRowClick={(row) => navigate(`/party/customers/${row.customer_id}`)}
            onEdit={(row) => navigate(`/party/customers/${row.customer_id}`)}
            onDelete={(row) => handleDelete(row.customer_id)}
            onAddRow={() => navigate('/party/customers', { state: { creating: true } })}
  ...
  const siteColumns = [
    { key: 'site_name', header: 'Site Name', width: '180px',
      ...
      ) : editingSiteIds.has(row.customer_site_id) ? (
        <Inp value={row.site_name || ''} onChange={v => siteUpdate(row.customer_site_id, { site_name: v })} placeholder="Name" />
    ...
    { key: 'site_number', header: 'Site Number', width: '130px',
      render: (row) => row.isNew ? (
        ...
        <Inp value={row.site_number || ''} onChange={v => siteUpdate(row.customer_site_id, { site_number: v })} placeholder="Number" />
    ...
    { key: 'address_id', header: 'Address', width: '200px',
      render: (row) => {
        ...
        const onChange = (e) => row.isNew ? setNewSite({ ...newSite, address_id: e.target.value }) : siteUpdate(row.customer_site_id, { address_id: e.target.value })
        const addr = addresses.find(a => a.address_id === val)
        const display = addr ? `${addr.address_line1 || ''}, ${addr.city || ''}` : '\u2014'
        return (row.isNew || editingSiteIds.has(row.customer_site_id)) ? (
    ...
    { key: 'is_primary', header: 'Primary', width: '90px',
      render: (row) => {
        ...
        const onChange = (v) => row.isNew ? setNewSite({ ...newSite, is_primary: v }) : siteUpdate(row.customer_site_id, { is_primary: v })
        return (row.isNew || editingSiteIds.has(row.customer_site_id)) ? (
    ...
    { key: 'is_active', header: 'Active', width: '80px',
      render: (row) => {
        ...
        const onChange = (v) => row.isNew ? setNewSite({ ...newSite, is_active: v }) : siteUpdate(row.customer_site_id, { is_active: v })
        if (row.isNew || editingSiteIds.has(row.customer_site_id)) return <Toggle value={val !== false} onChange={onChange} />
  ...
  const siteData = newSite ? [{ isNew: true, customer_site_id: 'new', ...newSite }, ...sites] : sites

  return (
    <div className="h-[calc(100vh-6.5rem)] flex flex-col">
      <div className="flex items-center justify-between px-4 py-3 border-b border-outline-variant">
        <div className="flex items-center gap-4">
          <button type="button" onClick={() => navigate('/party/customers', { replace: true })}
          ...
          <div>
            <h1 className="text-[32px] font-semibold leading-10 tracking-tight text-slate-900! dark:text-white!">
              {isEdit ? (existingCustomer?.customer_code || 'Edit Customer') : 'New Customer'}</h1>
            <p className="text-sm text-slate-500! dark:text-slate-300! mt-1">Configure customer details and sites</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="bg-surface-container rounded-xl border border-outline-variant overflow-hidden m-4">
          ...
          <div className="p-6">
            ...
            {headerTab === 'details' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <div><Fld label="Customer Code"><Inp value={form.customer_code} onChange={v => updateForm({ customer_code: v })} placeholder="Code" /></Fld></div>
                <div><Fld label="Customer Class"><Sel value={form.customer_class_code} onChange={v => updateForm({ customer_class_code: v })} options={CUSTOMER_CLASSES} /></Fld></div>
                <div><Fld label="Credit Limit"><Inp value={form.credit_limit} onChange={v => updateForm({ credit_limit: v })} type="number" placeholder="0.00" /></Fld></div>
                <div><Fld label="Payment Term (Days)"><Inp value={form.payment_term_days} onChange={v => updateForm({ payment_term_days: v })} type="number" placeholder="30" /></Fld></div>
                <div className="flex items-center gap-3 pt-6">
        ...
        {isEdit && (
          <div className="mx-4 mb-4">
            <InteractiveGrid
              title={`${sites.length} Customer Site(s)`}
              columns={siteColumns}
              data={siteData}
              idKey="customer_site_id"
              searchable
              selectedIds={selectedSiteIds}
              onSelect={(id) => { const n = new Set(selectedSiteIds); n.has(id) ? n.delete(id) : n.add(id); setSelectedSiteIds(n) }}
              onSelectAll={() => { if (selectedSiteIds.size === sites.length && sites.length > 0) setSelectedSiteIds(new Set()); else setSelectedSiteIds(new Set(sites.map(r => r.customer_site_id))) }}
              ...
              onDelete={(row) => { if (!row.isNew && confirm('Remove this site?')) siteDelete(row.customer_site_id) }}
              onEdit={(row) => { const n = new Set(editingSiteIds); n.add(row.customer_site_id); setEditingSiteIds(n) }}
              addLabel="Add Site"
